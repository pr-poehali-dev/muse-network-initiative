...
const MosaicGallery = ({ onViewingChange }: MosaicGalleryProps) => {
  ...
  const loadGallery = async () => {
    try {
      ...
      const galleryImages = data.items.map((item: any) => ({
        url: item.media_url,
        alt: item.title,
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center py-20">
        <div className="text-[#d4af37] text-lg">Загрузка галереи...</div>
      </div>
    );
  }
  ...
  if (images.length === 0) {
    return (
      <div className="flex items-center justify-center py-20">
        <div className="text-white/60 text-lg">Галерея пуста</div>
      </div>
    );
  }
