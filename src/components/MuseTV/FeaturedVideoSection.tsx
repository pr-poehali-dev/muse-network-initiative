            ...
            <div className="grid md:grid-cols-2 gap-6 p-6">
              <div className="relative aspect-video rounded-xl overflow-hidden group-hover:scale-[1.02] transition-transform duration-500">
                ...
                <div className="absolute inset-0 bg-black/40 group-hover:bg-black/20 transition-colors duration-300 flex items-center justify-center">
                  <div className="w-16 h-16 bg-[#d4af37]/90 rounded-full flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                    <Icon name="Play" className="w-8 h-8 text-black ml-1" />
                  </div>
                </div>
                {metadata?.duration && (
                  <div className="absolute bottom-3 right-3 px-3 py-1 bg-black/80 backdrop-blur-sm rounded-lg text-white text-sm font-medium">
              ...
              <div className="flex flex-col justify-center">
                <Badge variant="outline" className="border-[#d4af37]/40 text-[#d4af37] bg-[#d4af37]/10 px-3 py-1 w-fit mb-4">
                  {randomPodcast.type || 'Подкаст'}
                </Badge>
                
                <h3 className="text-2xl md:text-3xl font-bold text-white mb-4 group-hover:text-[#d4af37] transition-colors duration-300">
                  {metadata?.title || randomPodcast.title || 'MUSE подкаст'}
                </h3>
                
                {metadata?.description && (
                  <p className="text-white/60 text-base mb-6 line-clamp-3">
                    {metadata.description}
                  </p>
                )}
                ...
                <div className="flex items-center gap-6 text-white/50">
                  {metadata?.views && (
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm">{formatViews(metadata.views)} просмотров</span>
                    </div>
                  )}
                  {metadata?.author && (
                    <div className="flex items-center gap-2">
                      <Icon name="User" className="w-4 h-4" />
                      <span className="text-sm">{metadata.author.name}</span>
                    </div>
                  )}
                </div>
              </div>
