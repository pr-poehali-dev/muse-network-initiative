...
const EventsSection = ({ visibleSections, onEventRegister }: EventsSectionProps) => {
  ...
  return (
    <section id="events" className={`relative py-16 sm:py-24 md:py-32 px-4 sm:px-6 md:px-8 transition-all duration-1000 ${visibleSections.has('events') ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          ...
          {events.map((event, index) => (
            ...
            >
              ...
              <CardContent className="p-6 sm:p-8 md:p-10 relative z-10">
                <div className="text-4xl sm:text-5xl md:text-6xl mb-4 sm:mb-6 transform group-hover:scale-110 transition-transform duration-300">{event.emoji}</div>
                <h3 className="text-xl sm:text-2xl md:text-3xl font-bold mb-3 sm:mb-4 text-transparent bg-clip-text bg-gradient-to-br from-[#d4af37] to-[#8b7355]">{event.title}</h3>
                <p className="text-white/80 text-base sm:text-lg mb-3 sm:mb-4 leading-relaxed">{event.description}</p>
                <p className="text-sm sm:text-base text-white/60 mb-4 sm:mb-6 leading-relaxed">{event.details}</p>
                <Button 
                  onClick={() => onEventRegister(event.title)}
                  className="w-full bg-gradient-to-r from-[#d4af37] to-[#8b7355] hover:from-[#b8953d] hover:to-[#6b5d42] text-black font-bold py-4 sm:py-5 md:py-6 rounded-xl transition-all duration-300 transform hover:scale-105 text-sm sm:text-base"
                >
                  Записаться
                </Button>
              </CardContent>
